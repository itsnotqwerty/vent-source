  ...
  extends Intercom
{
  ...
  public static RealIntercom create(Application paramApplication, String paramString1, String paramString2)
  {
  ...
  private void registerIdentifiedUser(Registration paramRegistration)
  {
    ...
    if ((this.userIdentity.registrationHasAttributes(paramRegistration)) && (paramRegistration.getAttributes() != null))
    {
      this.twig.i("We already have a registered user. Updating this user with the attributes provided.", new Object[0]);
      performUpdate(paramRegistration.getAttributes().toMap());
      return;
    }
  ...
  void registerUser(IntercomSettings paramIntercomSettings)
  {
    if (!TextUtils.isEmpty(paramIntercomSettings.getUserHash())) {
      setUserHash(paramIntercomSettings.getUserHash());
    }
    if (paramIntercomSettings.canIdentifyUser())
    {
      Registration localRegistration = Registration.create();
      if (!TextUtils.isEmpty(paramIntercomSettings.getUserId())) {
        localRegistration.withUserId(paramIntercomSettings.getUserId());
      }
      if (!TextUtils.isEmpty(paramIntercomSettings.getEmail())) {
        localRegistration.withEmail(paramIntercomSettings.getEmail());
      }
      registerIdentifiedUser(localRegistration);
      return;
    }
  ...
  public void updateUser(UserAttributes paramUserAttributes)
  {
    ...
    if ((this.userIdentity.isUnidentified()) && (UserMergeUtils.hasIdentificationData(paramUserAttributes.toMap())))
    {
      Registration localRegistration = new Registration();
      UserMergeUtils.addIdentityDataToRegistration(localRegistration, paramUserAttributes.toMap());
      localRegistration.withUserAttributes(paramUserAttributes);
      registerIdentifiedUser(localRegistration);
      return;
    }
...
/* Location:              /home/qerwtr546/Documents/Vent Source/dex2jar-2.0/classes-dex2jar.jar!/io/intercom/android/sdk/experimental/RealIntercom.class
