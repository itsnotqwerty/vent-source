  ...
  implements Closeable
{
    ...
    throws IOException
  {
    ...
    f localf = f.cTY;
    if (paramInt1 > 0) {
      localf = this.source.ac(paramInt1);
    }
    ...
    throws IOException
  {
    ...
    {
      paramHandler = this.source.ac(Http2.CONNECTION_PREFACE.size());
      if (logger.isLoggable(Level.FINE)) {
        logger.fine(Util.format("<< CONNECTION %s", new Object[] { paramHandler.KJ() }));
      }
      if (!Http2.CONNECTION_PREFACE.equals(paramHandler)) {
        throw Http2.ioException("Expected a connection header but was %s", new Object[] { paramHandler.KE() });
      }
    }
  }
    ...
    implements s
  {
      ...
      throws IOException
    {
      ...
      do
      {
        ...
        break;
        paramLong = this.source.read(paramc, Math.min(paramLong, this.left));
      } while (paramLong == -1L);
...
/* Location:              /home/qerwtr546/Documents/Vent Source/dex2jar-2.0/classes-dex2jar.jar!/io/intercom/okhttp3/internal/http2/Http2Reader.class
